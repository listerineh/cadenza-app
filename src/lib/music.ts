...
import { NOTES, STANDARD_TUNING, CIRCLE_OF_FIFTHS_MAJOR, CIRCLE_OF_FIFTHS_MINOR, KEY_SIGNATURES, NOTE_MAP, INTERVAL_NAMES, SCALES, CHORD_LOOKUP } from './constants';
...
export const recognizeChord = (notes: string[]): RecognizedChord | null => {
  ...
  for (const rootNote of uniqueNotes) {
    ...
    if (quality) {
      ...
      }).sort((a,b) => (getNoteIndex(a.note) - rootIndex + 12) % 12 - (getNoteIndex(b.note) - rootIndex + 12) % 12);
