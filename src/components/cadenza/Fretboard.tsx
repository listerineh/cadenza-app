        ...
        <div style={{ minWidth: '800px' }}>
          ...
          <div className="flex flex-col-reverse">
            {strings.map((stringIndex) => {
              const selectedFretForString = selectedNotes.find(n => n.string === stringIndex);
              ...
              return (
                <div key={stringIndex} className="flex items-center">
                  ...
                  <div className="w-12 h-10 flex items-center justify-center pr-2 shrink-0">
                     <button
                        ...
                        className={cn(
                          ...
                          selectedFretForString?.fret === 0 ? "bg-primary border-primary/80 text-primary-foreground shadow-lg" : "bg-muted border-border",
                          isInteractive && "hover:bg-accent/50",
                          !isInteractive && "cursor-default"
                        )}
                        aria-label={`Play open string ${stringIndex + 1}`}
                      >
                        {selectedFretForString?.fret !== 0 && <X className="w-4 h-4 text-muted-foreground" />}
                        {selectedFretForString?.fret === 0 && <span className="font-bold text-sm">{getNoteFromFret(stringIndex, 0).note}</span>}
                  ...
                  <div className="flex-grow h-10 relative flex items-center">
                    <div className={cn("absolute w-full h-px bg-foreground/30", `z-0 h-[${stringIndex * 0.5 + 1}px]`)}></div>
                    ...
                    <div className="absolute w-full h-full flex z-10">
                      ...
                      {fretNumbers.slice(1).map(fret => {
                        ...
                        const isHighlighted = highlightedNotes.some(n => n.string === stringIndex && n.fret === fret);
                        const isRoot = (isSelected || isHighlighted) && rootNote === noteInfo.note;

                        let dotColor = isInteractive ? 'bg-transparent group-hover:bg-primary/20' : 'bg-transparent';
                        
                        if (isSelected) {
                          dotColor = isRoot ? 'bg-primary border-2 border-primary-foreground text-primary-foreground' : 'bg-primary border-2 border-primary/80';
                        } else if (isHighlighted) {
                          dotColor = isRoot ? 'bg-primary border-2 border-primary-foreground text-primary-foreground' : 'bg-accent border-2 border-accent/80';
                        }
                        
                        return (
                          <div key={fret} className="flex-1 h-full relative flex items-center justify-center">
                            <div className={cn("absolute top-0 right-0 h-full bg-foreground/20", fret === 0 ? 'w-2' : 'w-0.5')}></div>

                            <button
                              onClick={() => isInteractive && onFretToggle(stringIndex, fret)}
                              className={cn("w-full h-full flex items-center justify-center", isInteractive && 'group', !isInteractive && "cursor-default")}
                            ...
                            >
                               <div className={cn(
                                ...
                                dotColor,
                                (isSelected || (isHighlighted && !isRoot)) && "text-primary-foreground",
                            ...
                            {INLAY_FRETS.includes(fret) && (
                              <div className={cn("absolute flex justify-center w-full z-0",
                                stringIndex === 2 ? 'items-center' : (stringIndex > 2 ? 'items-end' : 'items-start')
                              )}>
                                  <div className={cn(
                                    "rounded-full bg-muted/40",
                                    fret === 12 && stringIndex === 2 ? "w-4 h-4" : "w-3 h-3",
                                    fret === 12 && (stringIndex === 0 || stringIndex === 4) ? "w-3 h-3" : "",
                                    INLAY_FRETS.includes(fret) && fret !== 12 && stringIndex === 2 ? "w-3 h-3" : ""
                                  )}></div>
                              </div>
                            )}
                          </div>
