        ...
        <div className="text-center bg-muted/30 p-6 rounded-lg flex flex-col justify-center border min-h-[140px]">
          {hasSelection ? (
            <>
              ...
              {chordData && !isUnknown && (
                <p className="text-muted-foreground mt-2 font-mono">{chordData.quality}</p>
              )}
            </>
          ) : (
          ...
          {hasSelection && chordData ? (
            <div className="space-y-2">
              {chordData.tones.map(({note, interval}) => (
                <div key={note} className="flex justify-between items-center text-sm bg-background p-2 rounded-md border">
