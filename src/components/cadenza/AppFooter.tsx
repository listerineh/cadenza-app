...
export default function AppFooter() {
    ...
    const NavLinkContent = ({ link, tNav, tHome }: { link: (typeof NAV_LINKS)[0], tNav: any, tHome: any }) => {
        const badgeContent = link.badge === 'new'
        ...
        const badge = link.badge && (
            <Badge
                size="icon"
                className={cn("absolute -bottom-1 -right-4 bg-transparent border-none")}
            >
                {badgeContent}
            </Badge>
        );
            ...
            <div className="container flex flex-col items-center justify-center gap-6">
                <div className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 text-center md:text-left">
                    <div className="flex flex-col items-center md:items-start gap-3">
                        ...
                        {NAV_LINKS.map(link => (
                            <Link key={link.href} href={link.href} className="text-muted-foreground transition-colors hover:text-foreground text-sm pb-1 relative">
