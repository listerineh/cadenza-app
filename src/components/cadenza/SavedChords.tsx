...
export default function SavedChords({ savedChords, onDelete, onSelect }: SavedChordsProps) {
  const { t } = useLanguage();
          ...
          {savedChords.length > 0 ? (
            <div className="space-y-3 pr-4">
              {savedChords.map((chord) => (
                <div key={chord.date} className="p-3 border rounded-md flex items-center justify-between hover:bg-muted/30 transition-colors">
                  <div className="flex items-center gap-4">
                     {chord.instrument === 'guitar' ? <Guitar className="h-6 w-6 text-muted-foreground" /> : <Piano className="h-6 w-6 text-muted-foreground" />}
                    <div>
                      <p className="font-bold text-lg font-mono text-primary">{chord.name}</p>
                      <p className="text-sm text-muted-foreground font-mono">{formatFingering(chord.fingering)}</p>
                    </div>
                  </div>
                  <div className="flex gap-1">
                     <Button variant="ghost" size="icon" onClick={() => onSelect(chord.fingering, chord.instrument)}>
                      <Music className="h-4 w-4" />
                       <span className="sr-only">{tSaved.loadChord}</span>
                    </Button>
                    <Button variant="ghost" size="icon" onClick={() => onDelete(chord.date)}>
